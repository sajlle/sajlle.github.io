(() => {
  ...
  function pageLooksDark() {
    ...
    const bodyClass = body ? (body.className || '').toLowerCase() : '';
    const classText = `${htmlClass} ${bodyClass}`;

    if (htmlTheme.includes('dark') || bodyTheme.includes('dark')) return true;
    if (classText.includes('dark')) return true;
  ...
  function getActivePalette() {
    return pageLooksDark() ? CONFIG.palettes.dark : CONFIG.palettes.light;
  }

  function getGlowColors() {
    if (pageLooksDark()) {
      return {
        inner: 'rgba(180, 220, 255, 0.22)',
  ...
  function spawnBurst(x, y) {
    ...
    const count = isMobile() ? CONFIG.desktopParticles : CONFIG.desktopParticles;
    const actualCount = isMobile() ? CONFIG.mobileParticles : CONFIG.desktopParticles;
    const ringColor = pick(palette);

    rings.push(new Ring(x, y, ringColor));

    for (let i = 0; i < actualCount; i++) {
      const angle = (Math.PI * 2 * i) / actualCount + random(-0.12, 0.12);
